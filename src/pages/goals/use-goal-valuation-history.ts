...
export function useGoalValuationHistory(
  ...
  // Determine date range (from goal creation to due date)
  const dateRange = useMemo(() => {
    if (!goal) return null;

    // Use a reasonable start date - either when goal was conceptually created
    // For now, we'll use 1 year ago as default start if no explicit creation date
  ...
  const chartData = useMemo((): GoalChartDataPoint[] => {
    ...
    const dateIntervals = generateDateIntervals(startDate, endDate, period);
    ...
    // Find starting value (first actual value, or 0)
    const sortedActualDates = Array.from(actualValuesByDate.keys()).sort();
    const startValue =
      sortedActualDates.length > 0 ? (actualValuesByDate.get(sortedActualDates[0]) ?? 0) : 0;

    // Build chart data points
    return dateIntervals.map((date) => {
      const dateStr = format(date, "yyyy-MM-dd");
      const monthsFromStart = getMonthsDiff(startDate, date);
      ...
      // Get actual value (only for dates up to today)
      const isInPast = isBefore(date, today) || isEqual(date, today);
      const actual = isInPast ? (aggregatedActuals.get(dateStr) ?? null) : null;
